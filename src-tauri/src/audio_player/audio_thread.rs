...
impl<'a> AudioThread<'a> {
    ...
    fn on_no_command_received(&self) {
        if !self.should_wait_for_command {
            if self.sink.len() == 0 {
                self.state.audio_player.play_next_track();
            }
        }
    ...
    fn update_now_playing_data(&self, sink: &Sink) {
        let playing_track_index = self.state.audio_player.playing_track_index.read().expect("playing_track_index should have been read");
        let music_queue = self.state.audio_player.music_queue.read().expect("music_queue should have been read");
        ...
        self.app_handle.emit("now_playing_changed", now_playing_data).unwrap();
    }
}